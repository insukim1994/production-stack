    ...
    while True:
        ...
        for endpoint in endpoints:
            ...
            if url in engine_stats:
                es = engine_stats[url]
                logstr += (
                    f" Engine Stats: Running Requests: {es.metrics['vllm:num_requests_running']}, "
                    f"Queued Requests: {es.metrics['vllm:num_requests_waiting']}, "
                    f"GPU Cache Hit Rate: {es.metrics['vllm:gpu_cache_usage_perc']:.2f}\n"
                )
            else:
